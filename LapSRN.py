...
class Model(object):
  ...
  def model(self):
    ...
    conv = tf.contrib.layers.conv2d(self.images, d, size, 1, 'VALID', 'NHWC', activation_fn=None,
                            weights_initializer=tf.variance_scaling_initializer(2), scope='feature_layer')
            ...
            for ri in range(r):
                for i in range(1, m+1):
                    ...
                    conv = tf.contrib.layers.conv2d(conv, d, 3, 1, 'SAME', 'NHWC', activation_fn=None, reuse=reuse,
                            weights_initializer=tf.variance_scaling_initializer(2), scope='embedding_layer{}'.format(i))
                conv = tf.add(conv, features)
                scope.reuse_variables()

        conv = tf.nn.leaky_relu(conv)
        conv = tf.contrib.layers.conv2d_transpose(conv, d, 4, 2, 'SAME', 'NHWC', activation_fn=None,
                            weights_initializer=tf.variance_scaling_initializer(2), reuse=reuse, scope='upsampling_layer')

        res = tf.contrib.layers.conv2d(conv, 1, 3, 1, 'SAME', 'NHWC', activation_fn=None,
                            weights_initializer=tf.variance_scaling_initializer(2), reuse=reuse, scope='res_layer')
